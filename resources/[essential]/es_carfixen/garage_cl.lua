...
Citizen.CreateThread(function ()
	...
	for i = 1, #vehicleRepairStation do
		...
		AddTextComponentString('Réparation')
...
function FixIt()
	if fixNeeded then
		...
		if not fixInProg then
			DrawSpecialText("Cout de reparation :~h~~y~ "..math.ceil(amount).." $, appuyez sur ~g~ENTRÉE~s~ pour valide", 1000)
			--ShowMoney(amount)
		...
		elseif fixInProg then
			DrawSpecialText("Reparation en cours. ~h~~y~Merci de patiente~w~.", repairTime)
			for i = 1, amount do
				Wait(100)
